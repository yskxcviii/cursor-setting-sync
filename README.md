...
- [Node.js](https://nodejs.org/) (v16 以上。`npm` が Node バージョン不一致を警告する場合は、Node 20 LTS または 22.9 以降の利用を推奨します)
- [Git](https://git-scm.com/)
- Windows の場合: 開発者モードが有効であること (シンボリックリンク作成に必要)
- Windows の場合: `sync:enable` はタスクスケジューラに定期 pull 用タスクを登録します。`アクセスが拒否されました ( 0x80070005 )` などで失敗する場合は、**管理者として** PowerShell またはターミナルを開き、再度 `npm run sync:enable` を実行してください。組織ポリシーでタスク作成が制限されている場合は、管理者に許可を依頼してください。
...
- **Windows のタスクスケジューラから `pull.mjs` が実行される場合、コンソールやターミナルにはログは表示されません** (バックグラウンド実行のため)。
