...
Cursor の `settings.json` を複数デバイス間で同期するためのユーティリティです。

## 仕組み

- リポジトリ内の `settings.json` を Cursor の設定ディレクトリにシンボリックリンクとして配置します
- 毎時 00 分に `main` ブランチを自動で pull し、最新の設定を反映します
...
- Windows の場合
    - 開発者モードが有効であること (シンボリックリンク作成に必要)
    - `sync:enable` はタスクスケジューラに定期 pull 用タスクを登録します。`アクセスが拒否されました (0x80070005)` などで失敗する場合は、**管理者として** PowerShell またはターミナルを開き、再度 `npm run sync:enable` を実行してください。組織ポリシーでタスク作成が制限されている場合は、管理者に許可を依頼してください。
...
3. 自動 pull をバックグラウンドタスクとして登録 (Mac: launchd / Windows: タスクスケジューラ)
    ...
    - 以降、毎時 00 分に `main` ブランチを pull します
